...
type ModelConfiguration = Pick<ModelWorkerConfiguration, "kind" | "modelCacheDir" | "modelId" | "ffmpegPath" | "ffprobePath"> & { allowModelDownload?: boolean };
...
export async function isolatedModelCall(configuration: ModelConfiguration, call: ModelWorkerCall, options: MediaOperationOptions): Promise<ModelWorkerValue> {
  ...
  return pool.request({
    ...
    modelId: configuration.modelId?.startsWith(".") ? resolve(configuration.modelId) : configuration.modelId,
