...
import { ModelFileResolver } from "./model-file-resolver.js";
...
let busy = false;
let resolver: ModelFileResolver | undefined;
let cacheStaging: string | undefined;
...
process.on("disconnect", () => {
  ...
  if (cacheStaging) rmSync(cacheStaging, { recursive: true, force: true, maxRetries: 3 });
...
async function execute(request: ModelWorkerRequest): Promise<ModelWorkerResponse> {
  try {
    ...
    if (!provider) {
      ...
      resolver = new ModelFileResolver(config, workerRoot!);
      cacheStaging = modelCacheStagingRoot(config.modelCacheDir, workerRoot!);
      Object.assign(env, { videobookModelFileResolver: resolver.get.bind(resolver) });
    ...
    if (resolver?.failure) throw resolver.failure;
    ...
    const integrityFailure = await resolver?.drainAfterFailure();
    return { id: request.id, ok: false, error: modelWorkerError(integrityFailure ?? error) };
  }
}
