...
const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), 'clipfirst-git-'));
...
try {
  ...
  // --- Write version 1 ---
  const w1 = await cfs.writeFile(assetId, 'script.txt', 'Version 1: rough draft', slug);
  if (!w1.ok) throw new Error(w1.error.message);
  console.log('Wrote script.txt v1');

  // --- Write version 2 ---
  const w2 = await cfs.writeFile(assetId, 'script.txt', 'Version 2: revised and polished', slug);
  if (!w2.ok) throw new Error(w2.error.message);
  console.log('Wrote script.txt v2');

  // --- Write another file ---
  const w3 = await cfs.writeFile(assetId, 'notes.txt', 'Some production notes', slug);
