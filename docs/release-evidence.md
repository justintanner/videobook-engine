...
| Argument arrays, bounded outputs, timeouts, cancellation and scoped workspaces; no untrusted shell | Baseline engine collectors were unbounded. Subsequent shared media-process helper adds byte caps, deadlines, SIGKILL cancellation and safe typed errors; real child-process and cached-media E2E tests pass. | Bounded decoder paths completed; consumer signal forwarding and model process isolation remain in `ve-ovz.13`. See `docs/media-limits.md`. |
| Malformed codec, oversized image, decompression bomb and model OOM fail job without book corruption | Subsequent tests reject malformed audio/images, >64 MiB files and >40M-pixel inputs, then verify retry and preserved source/coverage. | Native/model OOM containment and full queue integration still require proof in `ve-ovz.13`. |
