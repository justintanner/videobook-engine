...
Utils.prototype.makeDir = function (/*String*/ folder, /*String*/ root) {
    const self = this;
    const lstatSync = typeof self.fs.lstatSync === "function" ? self.fs.lstatSync.bind(self.fs) : self.fs.statSync.bind(self.fs);
    // only components strictly inside the extraction root are attacker-controlled
    // (CVE-2026-76845); root itself and anything above it is trusted caller input
    const resolvedRoot = root ? pth.resolve(pth.normalize(root)) : null;
    ...
    function mkdirSync(/*String*/ fpath) {
        ...
        fpath.split(self.sep).forEach(function (name) {
            ...
            var stat;
            const insideRoot = !resolvedRoot || (resolvedPath.length > resolvedRoot.length && resolvedPath.startsWith(resolvedRoot + self.sep));
            try {
                stat = insideRoot ? lstatSync(resolvedPath) : self.fs.statSync(resolvedPath);
            } catch (e) {
            ...
            if (stat && insideRoot && stat.isSymbolicLink && stat.isSymbolicLink()) throw Errors.FILE_IN_THE_WAY(`"${resolvedPath}"`);
...
Utils.prototype.writeFileTo = function (/*String*/ path, /*Buffer*/ content, /*Boolean*/ overwrite, /*Number*/ attr, /*String*/ root) {
    const self = this;
    const lstatSync = typeof self.fs.lstatSync === "function" ? self.fs.lstatSync.bind(self.fs) : self.fs.statSync.bind(self.fs);

    // reject a pre-existing symlink anywhere strictly inside the extraction root,
    // so a crafted entry name cannot redirect a write outside of it (CVE-2026-76845).
    // Ancestors of root (e.g. macOS's /var -> /private/var) are trusted caller input
    // and intentionally left unchecked to avoid false positives.
    if (root) {
        const resolvedRoot = pth.resolve(pth.normalize(root));
        if (path.length > resolvedRoot.length && path.startsWith(resolvedRoot + self.sep)) {
            let resolvedPath = resolvedRoot;
            const pathParts = path.slice(resolvedRoot.length + self.sep.length).split(self.sep);
...
Utils.prototype.writeFileToAsync = function (/*String*/ path, /*Buffer*/ content, /*Boolean*/ overwrite, /*Number*/ attr, /*Function*/ callback, /*String*/ root) {
    ...
    // reject a pre-existing symlink anywhere strictly inside the extraction root
    // (CVE-2026-76845); see the sync counterpart in writeFileTo for details.
    const checkSymlink = function (done) {
        if (!root || typeof self.fs.lstat !== "function") return done(true);

        const resolvedRoot = pth.resolve(pth.normalize(root));
        if (!(path.length > resolvedRoot.length && path.startsWith(resolvedRoot + self.sep))) return done(true);
