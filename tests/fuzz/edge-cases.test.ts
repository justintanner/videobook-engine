...
describe('edge-cases fuzz tests', () => {
  ...
  it('createAsset handles 100+ slug collisions without hanging', async () => {
    ...
    for (let i = 0; i < 105; i++) {
      const result = await sandbox.fs.createAsset('vid', 'collision', projectSlug);
  ...
  it('readMetadata for nonexistent file returns NOT_FOUND', async () => {
    const asset = await sandbox.fs.createAsset('img', 'no-metadata', projectSlug);
    expect(asset.ok).toBe(true);
    if (!asset.ok) throw new Error('Failed to create asset');

    const result = await sandbox.fs.readMetadata(asset.value.assetId, projectSlug);
    expect(result.ok).toBe(false);
    if (!result.ok) {
      expect(result.error.code).toBe('NOT_FOUND');
    }
  ...
  it('createAsset always produces safe assetId (property test, 30 runs)', async () => {
    await fc.assert(
      fc.asyncProperty(validPrefixArb, safeAssetNameArb, async (prefix, name) => {
        const result = await sandbox.fs.createAsset(prefix, name, projectSlug);
        expect(result.ok).toBe(true);
        if (result.ok) {
  ...
  it('normal rename works (positive case for rollback context)', async () => {
    const asset = await sandbox.fs.createAsset('vid', 'original-name', projectSlug);
    expect(asset.ok).toBe(true);
    if (!asset.ok) throw new Error('Failed to create asset');
    const oldId = asset.value.assetId;

    const renameResult = await sandbox.fs.renameAsset(oldId, 'new-fancy-name', projectSlug);
