...
describe('git-integrity fuzz tests', () => {
  ...
  it('writeMetadata produces a git commit', async () => {
    // Create an asset
    const assetResult = await sandbox.fs.createAsset('vid', 'meta-test', projectSlug);
    if (!assetResult.ok) throw new Error('Failed to create asset');
    const assetId = assetResult.value.assetId;

    // Write metadata
    const metaResult = await sandbox.fs.writeMetadata(assetId, { origin: 'test', width: 1920 }, projectSlug);
    expect(metaResult.ok).toBe(true);

    // Verify a commit with 'metadata' in the message exists
    const history = await sandbox.fs.getHistory(projectSlug);
    const metadataCommit = history.find((c) => c.message.includes('metadata'));
    expect(metadataCommit).toBeDefined();
    expect(metadataCommit!.hash).toBeTruthy();
  });

  it('commit messages with pipe chars do not corrupt history', async () => {
    const assetResult = await sandbox.fs.createAsset('vid', 'pipe-test', projectSlug);
    if (!assetResult.ok) throw new Error('Failed to create asset');
    const assetId = assetResult.value.assetId;

    // Write a file with pipe in the filename
    const writeResult = await sandbox.fs.writeFile(assetId, 'data|pipe.txt', 'content', projectSlug);
    ...
    if (!writeResult.ok) {
      ...
      await sandbox.fs.commitOperation('write|pipe|test', assetId, undefined, projectSlug);
    }
    ...
    await fc.assert(
      fc.asyncProperty(commitMessageInjectionArb, async (fuzzedMsg) => {
        ...
        try {
          ...
          const assetResult = await propSandbox.fs.createAsset('vid', 'fuzz', slug);
          ...
          await propSandbox.fs.commitOperation(fuzzedMsg, assetId, undefined, slug);
  ...
  it('commitOperation with no changes returns null', async () => {
    ...
    const result = await sandbox.fs.commitOperation('noop', undefined, undefined, projectSlug);
    expect(result).toBeNull();
  });

  it('commitOperation does not stage bystander files', async () => {
    // Create two assets
    const asset1Result = await sandbox.fs.createAsset('vid', 'bystander-one', projectSlug);
    if (!asset1Result.ok) throw new Error('Failed to create asset 1');
    const asset1Id = asset1Result.value.assetId;

    const asset2Result = await sandbox.fs.createAsset('vid', 'bystander-two', projectSlug);
    ...
    await fs.writeFile(path.join(asset1Dir, 'uncommitted.txt'), 'should stay unstaged');

    // Write a file to asset-2 and commit via commitOperation scoped to asset-2
    const asset2Dir = path.join(projectDir, asset2Id);
    await fs.writeFile(path.join(asset2Dir, 'committed.txt'), 'should be committed');
    const hash = await sandbox.fs.commitOperation('write', asset2Id, undefined, projectSlug);
    ...
    const asset2InStatus = lines.some((l) => l.includes(asset2Id) && l.includes('committed.txt'));
    expect(asset2InStatus).toBe(false);
  });

  it('rewindToCommit with invalid hash returns Result error', async () => {
    const projectDir = path.join(sandbox.outputDir, projectSlug);
    const result = await rewindToCommit(projectDir, 'deadbeef1234567890abcdef1234567890abcdef');
