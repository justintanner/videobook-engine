...
describe('e2e full API tour', () => {
  ...
  it('walks all 23 public methods in one happy-path flow', async () => {
    ...
    // 8. writeFile
    const writeResult = await cfs.writeFile(assetId, 'original.mp4', MINIMAL_MP4, alphaSlug);
    ...
    // 10. writeMetadata
    const meta = { origin: 'veo', width: 1920, height: 1080, duration: 12.5 };
    const writeMeta = await cfs.writeMetadata(assetId, meta, alphaSlug);
    expect(writeMeta.ok).toBe(true);

    // 11. readMetadata
    const readMeta = await cfs.readMetadata(assetId, alphaSlug);
    ...
    // 14. acquireLock
    const lock = await cfs.acquireLock(assetDir, '.generating.lock', { task_id: 'test-123' });
    ...
    // 18. cleanStaleLocks — write a stale lock with dead PID
    const staleLockPath = path.join(assetDir, '.generating.lock');
    const staleLockData = JSON.stringify({ created_at: Date.now(), pid: 999999, task_id: 'stale' });
    ...
    // 19. commitOperation — write a file bypassing auto-commit, then commit manually
    const notesPath = path.join(assetDir, 'notes.txt');
    await fs.writeFile(notesPath, 'some annotation');
    const commitHash = await cfs.commitOperation('annotate', assetId, { note: 'added notes' }, alphaSlug);
    expect(commitHash).toBeTruthy();
    expect(typeof commitHash).toBe('string');

    // 20. Overwrite original.mp4 with v2 data
    const V2_MP4 = Buffer.concat([MINIMAL_MP4, Buffer.from([0xde, 0xad, 0xbe, 0xef])]);
    const writeV2 = await cfs.writeFile(assetId, 'original.mp4', V2_MP4, alphaSlug);
    ...
    expect(assetHistory.length).toBeGreaterThanOrEqual(5);

    // 23. restoreAsset — history is newest-first; pick a commit before the v2 write
    // Index 0 = v2 write, 1 = annotate, 2 = metadata write (has v1 mp4)
    ...
    const readRenamed = await cfs.readFile(renamedId, 'original.mp4', alphaSlug);
    ...
    // 26. slugTaken — the original assetId was used historically
    const taken = await cfs.slugTaken(assetId, alphaSlug);
    expect(taken).toBe(true);

    // 27. slugTaken — never-used slug
